...
# Guards are not listed here: scripts/check_all_guards.sh discovers them by filename, and CI
# invokes that same script, so the local gate and CI share one discovery rule and cannot drift.
# To add a guard, name it scripts/check_*.{lean,py,sh} — do not edit this file.
