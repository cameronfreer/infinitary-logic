...
The formalization establishes eleven headline theorems:
\begin{itemize}
...
The node below is the \emph{relational core} --- the arc's first publishable
theorem.  It is proved over a purely relational language with no countability
hypothesis, by an inseparability/finite-condition Henkin construction (a
constant-free interpolant strips through the empty-support root gate), with
ambient relation-symbol countability removed by passing to the sublanguage
generated by the two roots' countably many symbols.  Its disjunction form is the
PC-separation corollary \texttt{craig\_pcSeparation\_relational}.  The full,
arbitrary-language theorem (function symbols in the occurrence bound) follows
from a relationalization layer --- function symbols translated to graph
relations --- that is a self-contained syntactic translation still under
construction.
