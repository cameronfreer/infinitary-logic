...
\begin{itemize}
  ...
  \item \textbf{The exact Hanf number.}
    ...
    sharp, by Marker's Exercise~5.0.6~\cite{marker2016} beth ladder over the von Neumann hierarchy
...
Exercise~5.0.6: for each $\alpha < \omegaone$ a single $\Lomegaone$ sentence
...
\emph{Status: proved.}  Marker's Theorem~5.2.5~\cite{marker2016}
...
The former full-fragment bridge (a ``full Barwise fragment'' carrying every
sentence and a chain-closure hypothesis on $A$-consistency, with a completeness
theorem over it) has been retired: chain closure fails for the full universe of a
constants-expanded language, where the $\omega$-rule derives $\bot$ from the union
of a chain of consistent sets, so that structure was uninhabited there.  The
successors --- the countable-completion kernel over a Henkin-closed sentence set,
the source-fragment adapter in the constants-expanded universe, and the
countable-signature graph-universe endpoint --- assume consistency in the
expanded or relationalized universe rather than in the base language, so no
successor is recorded here as a replacement of the retired completeness node.
...
route (Theorem~4.3.7 of~\cite{marker2016}; Gao~\cite[Theorem~11.3.6]{gao2009} is an
alternative exposition) rather than an induction on the Borel
...
\emph{Status: proved.}  The payoff of the small-model theorem (Marker, Theorem~5.2.5
...
\section{Thinness and Perfect Sets}\label{sec:thinness}
...
\texttt{ThinRankAnalysis.isThinOn} proves the implication.  Note that the cheap
direction above does not weaken those hypotheses: \texttt{isThinOn} consumes
perfect $\to$ Cantor, the complete-metric direction.

Thinness also has a purely logical characterization, with no rank at all: a
Borel class carries no perfect antichain exactly when every countable list of
sentences realizes only countably many truth patterns on it.  One direction is
Silver's dichotomy applied to equality of truth sequences, a Borel equivalence
relation coarser than isomorphism; the other recovers the Cantor parameter of an
antichain by sentences, through invariant analytic separation and
L\'opez--Escobar.  The class need not be isomorphism-invariant, and no
Borelness of isomorphism is used (classical background: invariant separation and
L\'opez--Escobar are Corollary~4.3.6 and Theorem~4.3.7 of Marker~\cite{marker2016},
scatteredness is his Definition~3.3.1 and Corollary~3.3.10; the iterated
separation-and-saturation argument is that of Gao~\cite[Lemma~5.4.6]{gao2009};
the spectrum formulation is assembled here).  Countable spectra
quantify over every list;
they do not supply one list that classifies isomorphism, which together with
thinness would force countably many classes.

\inputleannode{thm:thin-iff-countable-sentence-spectra}

The finite-arity form is the classical definition of scatteredness (Marker,
Definition~3.3.1 and Corollary~3.3.10): a Borel class is thin exactly when every
countable fragment realizes only countably many types of $n$-tuples on it, for
every finite $n$.  The route is Marker's Corollary~3.3.3: for a fixed fragment
and arity, ``realizing the same types'' is a Borel equivalence relation refined
by isomorphism, so Silver's dichotomy either bounds the realized types or
produces an isomorphism antichain.  No Baire-property theorem for analytic sets
is needed.

\inputleannode{thm:thin-iff-countable-fragment-spectra}

A generic constructor of \texttt{ThinRankAnalysis} is supplied, from ranks computed
by coded well-orders presented continuously on each Cantor antichain
(Definition~\ref{def:wellorder-presentation-thinness}); it is the bridge from
analytic boundedness to thinness.  No \emph{concrete} instance is supplied: no
model class is exhibited whose isomorphism rank admits such presentations, and
producing one is the remaining mathematics rather than a packaging step.

The refined form of the boundedness field --- a bound on a continuous injective
\emph{subcopy} of each Cantor antichain rather than on the antichain itself ---
is what makes Borel data along the antichain usable: countably many maps from
Cantor space into countable discrete spaces with Borel fibres become
simultaneously continuous after composing with one continuous injective
self-map of Cantor space
(\texttt{CantorStabilization.exists\_subcopy\_continuous}, by the Baire
property of Borel sets and the fact that an uncountable Borel subset of a
Polish space contains a Cantor copy).  A construction may therefore choose the
subcopy first and then treat such data as continuous.
...
Marker's consistency-property construction (Theorem~4.4.1 and Corollary~4.4.2
...
fifteen closure fields are Exercise~4.28 of the 2013 notes.  The fair Henkin enumeration and the quotient
...
so every model of the conjunction is in fact well-ordered.  Corollary~4.4.2 then
...
The same boundedness reaches trees.  A code names a tree on $\mathbb{N}$ through a
distinguished unary relation, and the tree class is closed.  The Kleene--Brouwer
code sends a tree code to the code of the order that lists the tree's nodes first,
in Kleene--Brouwer order, and every other natural afterwards; each coordinate of the
code depends on two membership bits, so the map is continuous.  A well-founded tree
has a well-ordered Kleene--Brouwer code, and its rank is at most the order type, so
the bound on order types becomes a bound on tree ranks.

\inputleannode{thm:analytic-wellfounded-tree-boundedness}

The two boundedness theorems are Theorems~1.6.10 and~1.6.11 of Gao~\cite{gao2009},
whose Exercise~1.5.7 is the Kleene--Brouwer equivalence used to pass between them.

That boundedness is what the thinness package of Section~\ref{sec:thinness}
needs on a Cantor antichain, whenever the rank in question is the order type of
a continuously presented family of coded well-orders.  Packaging the two
together is the bridge between the chains, and it is generic: the presentation
is quantified \emph{inside} the antichain, so each antichain supplies its own
coding rather than the whole space carrying a uniform one.

\inputleannode{def:wellorder-presentation-thinness}
