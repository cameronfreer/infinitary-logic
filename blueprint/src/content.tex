...
\[
  \text{perfect antichain} \longrightarrow \text{Cantor antichain}
...
of that package is supplied here.  Note also that the conversion from a Cantor
antichain back to a perfect antichain is not currently proved, and that cardinal
equality alone does not establish thinness: a quotient of size $\aleph_1$ is
compatible both with thinness and with its failure under $\neg\mathsf{CH}$.
