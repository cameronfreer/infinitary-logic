...
- **Further model theory** &mdash; downward L&ouml;wenheim&ndash;Skolem for L<sub>&omega;<sub>1</sub>&omega;</sub>, Hanf numbers, conditional Morley&ndash;Hanf bound (conditional on `MorleyHanfTransfer`), and counting-models infrastructure.
- **Admissible fragments** &mdash; Barwise compactness, Barwise completeness II, and the Nadel bound.

Some results carry explicit hypotheses that package external mathematical content not yet formalized (e.g., `MorleyHanfTransfer` for Erd&#337;s&ndash;Rado / Ehrenfeucht&ndash;Mostowski machinery). The schematic `morley_counting_dichotomy` awaits descriptive set theory infrastructure.
