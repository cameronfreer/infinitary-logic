...
every `L_{ω₁ω}` sentence over a language with countably many function and relation symbols. All
inputs proved: tail extraction (`morleyHanfExtractionTail_holds`, countable Ramsey) and
seed-template realizability (the schema construction). The transparent intermediate to the
assumption-free `morley_hanf` below. -/
theorem morley_hanf_countable_symbols {L' : Language.{0, 0}}
    [Countable (Σ n, L'.Functions n)] [Countable (Σ n, L'.Relations n)]
    (φ : L'.Sentenceω) :
    IsHanfBound φ (Cardinal.beth (Ordinal.omega 1)) :=
  morley_hanf_of_tail_realizable morleySeedTailTemplateRealizable_of_countable_symbols φ
...
@[blueprint "thm:morley-hanf"
  ...
  (proof := /-- Tail extraction is countable Ramsey on the seed family. Seed-template
    realizability is the schema construction: the $\omega$-stage Marker/Henkin completion of the
    countable schema sentence universe (with pinned positive-disjunction and refuted-conjunct
    witnesses), its quotient term model with a restricted truth lemma, the fully indiscernible
    sequence of Henkin constants, and cross-source acceptance through a Skolem-universality
    mixin. Symbol countability is removed by running the construction in the sublanguage
    generated by the sentence's own (countably many) symbols and expanding the model back. -/)
