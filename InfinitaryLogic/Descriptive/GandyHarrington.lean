...
/-! ### Gandy-Harrington interface theorem -/

/-- **Gandy-Harrington topology theorem**: For any Polish space α, there exists
a Polish space Ω with a continuous injective map to α such that:
1. Every nonempty analytic subset of α meets the range of i.
2. For every analytic A ⊆ α × α, the pullback along (i × i) is open in Ω × Ω.

This is the abstract interface to the Gandy-Harrington construction. The proof
requires building the GH topology (generated by analytic sets) on α, finding
a dense Gδ Polish subspace, and verifying the two properties. -/
theorem gandy_harrington_interface {α : Type u}
    ...
    ∃ (Ω : Type u) (_ : MetricSpace Ω) (_ : CompleteSpace Ω)
      ...
      (∀ {s : Set α}, AnalyticSet s → s.Nonempty → (range i ∩ s).Nonempty) ∧
      (∀ {A : Set (α × α)}, AnalyticSet A →
        IsOpen ((fun p : Ω × Ω => ((i p.1), (i p.2))) ⁻¹' A)) := by
  sorry

/-! ### GH core existence (derived from interface) -/

/-- The GH core for a specific Borel equivalence relation: a Polish Y with
continuous injective map to α, closed pulled-back relation, AND every class
meets the image. Derived from `gandy_harrington_interface`. -/
private theorem gh_exists_core {α : Type u}
  ...
  obtain ⟨Ω, instM, instC, instS, i, hi_cont, hi_inj, hi_hits, hi_open⟩ :=
    @gandy_harrington_interface α _ _ _ _ _
  refine ⟨Ω, instM, instC, instS, i, hi_cont, hi_inj, ?_, ?_⟩
  · -- Closedness: E is Borel → Eᶜ is Borel → Eᶜ is analytic → pullback of Eᶜ is open → pullback of E is closed
    rw [← isOpen_compl_iff]
    have : (fun p : Ω × Ω => (i p.1, i p.2)) ⁻¹' {p : α × α | r.r p.1 p.2}ᶜ =
        ({p : Ω × Ω | r.r (i p.1) (i p.2)})ᶜ := by
      ext ⟨a, b⟩; simp [Set.mem_compl_iff, Set.mem_preimage, Set.mem_setOf_eq]
    rw [← this]
    exact hi_open (hr.compl.analyticSet)
  · -- Every class meets range i
    intro x
    have hne : {y : α | r.r x y}.Nonempty := ⟨x, r.iseqv.refl x⟩
    have hanalytic := class_analyticSet r hr x
    obtain ⟨z, ⟨y, rfl⟩, hz⟩ := hi_hits hanalytic hne
    exact ⟨y, r.iseqv.symm hz⟩
