...
transfer, now reduced to the single residual `TailTemplateRealizable`
(consumed by `morley_hanf_of_tail_realizable`; the extraction side is
discharged by `morleyHanfExtractionTail_holds`).
