...
This is the key bridge lemma. The proof uses the fact that the Scott formula
`scottFormula a α` is determined by codes: its truth value on any tuple `b` can
be computed from the codes that `b` satisfies. -/
